defmodule RedisZTest.Shards do
  ...
  describe "select_shard/2" do
    test "The two keys {user1000}.following and {user1000}.followers will hash to the same hash slot since only the substring user1000 will be hashed in order to compute the hash slot." do
    ...
    # test "For the key foo{}{bar} the whole key will be hashed as usually since the first occurrence of { is followed by } on the right without characters in the middle." do
    # end

    test "For the key foo{{bar}}zap the substring {bar will be hashed, because it is the substring between the first occurrence of { and the first occurrence of } on its right." do
      args = [shards: [[name: A], [name: B]]]
      assert Shards.select_shard("{bar", args) === Shards.select_shard("foo{{bar}}zap", args)
    end

    test "For the key foo{bar}{zap} the substring bar will be hashed, since the algorithm stops at the first valid or invalid (without bytes inside) match of { and }." do
      args = [shards: [[name: A], [name: B]]]
      assert Shards.select_shard("bar", args) === Shards.select_shard("foo{bar}{zap}", args)
    end

    # test "What follows from the algorithm is that if the key starts with {}, it is guaranteed to be hashed as a whole. This is useful when using binary data as key names."do
    # end
